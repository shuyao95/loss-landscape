...
def sepconv3x3(C_in, C_out, stride, affine=True):
		...
		for i in range(nodes):
			if i == 0:
				if reduction:
					...
					stride = 1
				for _ in range(self.prev_connects):
					op = sepconv3x3(channels, channels, stride)
					self.ops.append(op)
			else:
				op = sepconv3x3(channels, channels, stride=1)
				self.ops.append(op)
	...
	def forward(self, inputs, drop_prob=0):
		...
		for i in range(self.nodes):
			if i == 0:
				...
				out = self.ops[indices+i-1](outs[-1])
			outs.append(out)
