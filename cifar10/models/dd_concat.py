...
class Cell(nn.Module):
    def __init__(self, prev_channels, prev_connects, prev_reduction, nodes, channels, reduction):
        ...
        for i in range(self.prev_connects):
            if i > 0:
                stride = 2 * \
                    len(list(filter(lambda x: x is True, prev_reduction[:i])))
                if stride > 0:
                    op = FactorizedReduce(
                        prev_channels[i], channels, stride=stride)
                else:
        ...
        if reduction:
            self.op = nn.MaxPool2d(3, stride=2, padding=1)
        else:
    ...
    def forward(self, inputs, drop_prob=0):
        ...
        N, C, H, W = outs.size()
        outs = outs.view(N, self.channels,
                         self.prev_connects, self.nodes, H, W)
    ...
    def __init__(self, C, nodes, num_classes, layers, auxiliary, prev_connects=2):
        ...
        for i in range(layers):
            if i in [layers // 3, 2 * layers // 3]:
                ...
                reduction = False
            cell = Cell(prev_channels, prev_connects,
                        prev_reduction, nodes, C_curr, reduction,)
            self.cells += [cell]
            if i == 2 * layers // 3:
...
def dd_node1():
    return NetworkCIFAR(C=64, nodes=1, num_classes=10, layers=20, auxiliary=False,
                        prev_connects=2)
...
def dd_node4():
    return NetworkCIFAR(C=32, nodes=4, num_classes=10, layers=20, auxiliary=False,
                        prev_connects=2)


def dd_node5():
    return NetworkCIFAR(C=28, nodes=5, num_classes=10, layers=20, auxiliary=False,
                        prev_connects=2)


def dd_prev1():
    return NetworkCIFAR(C=45, nodes=4, num_classes=10, layers=20, auxiliary=False,
                        prev_connects=1)
...
def dd_prev3():
    return NetworkCIFAR(C=26, nodes=4, num_classes=10, layers=20, auxiliary=False,
                        prev_connects=3)


def dd_prev4():
    return NetworkCIFAR(C=22, nodes=4, num_classes=10, layers=20, auxiliary=False,
                        prev_connects=4)
